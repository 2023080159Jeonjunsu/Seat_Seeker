                    ...
                    try {
                        ...
                        if (documentSnapshot.exists()) {
                            ...
                            if (seat_f != null && reserv_f != null) {
                                Log.d("FirestoreData", "$seatID: seat_f=$seat_f, reserv_f=$reserv_f")
                                ...
                                runOnUiThread {
                                    if (seatID == "A1" && reserv_f == true) {
                                        ...
                                        btnA1.backgroundTintList = ColorStateList.valueOf(colorGreen) // 배경 이미지 리소스로 설정
                                    }
                                    if (seatID == "A2" && reserv_f == true) {
                                        val btnA2 = findViewById<Button>(R.id.btnA2)
                                        val colorGreen = resources.getColor(R.color.green)
                                        btnA2.backgroundTintList = ColorStateList.valueOf(colorGreen) // 배경 이미지 리소스로 설정
                                    }
                                    if (seatID == "A3" && reserv_f == false) {
                                        val btnA3 = findViewById<Button>(R.id.btnA3)
                                        val colorRed = resources.getColor(R.color.red)
                                        btnA3.backgroundTintList = ColorStateList.valueOf(colorRed) // 배경 이미지 리소스로 설정
                                    }
                                }
